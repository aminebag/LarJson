 ...
 *
 * A root implementation of a typed LarJson object
 */
public class LarJsonRootObject extends AbstractLarJsonObject implements LarJsonContext {
    ...
    public LarJsonRootObject(ObjectLarJsonPropertyMapper<?> mapper, long[] attributes, long jsonPosition,
                             LarJsonTypedReadConfiguration configuration,
                             ResourcePool<? extends LarJsonBlueprintReader> blueprintReaderPool,
                             ChannelCharacterStreamPool characterStreamPool, PropertyResolver propertyResolver,
                             Class<?> rootInterface) {
    ...
    @Override
    public <T> T getCachedValue(long jsonPosition, LarJsonValueCalculator<T> valueCalculator) throws IOException, LarJsonException {
    ...
    public void checkClosed() {
        if(closed.get()) {
            throw new IllegalStateException("The root object has been closed");
        }
    }
