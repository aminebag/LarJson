...
public class FloatPropertyMapper extends SimpleValuePropertyMapper<Float> {

    private final LarJsonValueParser larJsonValueParser;

    public FloatPropertyMapper(int index, LarJsonValueParser larJsonValueParser) {
        super(index);
        this.larJsonValueParser = larJsonValueParser;
    }

    @Override
    Float calculateValue(LarJsonBranch branch, long key) throws IOException, LarJsonParseException {
        return larJsonValueParser.parseFloat(branch.getCharacterStream(key));
    }
