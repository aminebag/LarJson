...
public class StringPropertyMapper extends SimpleValuePropertyMapper<String> {

    private final LarJsonValueParser larJsonValueParser;

    public StringPropertyMapper(int index, LarJsonValueParser larJsonValueParser) {
        super(index);
        this.larJsonValueParser = larJsonValueParser;
    }

    @Override
    String calculateValue(LarJsonBranch branch, long key) throws IOException, LarJsonParseException {
        return larJsonValueParser.parseString(branch.getCharacterStream(key));
    }
